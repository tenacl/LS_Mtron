...
def get_prompt_text(track, topic, purpose=None, sources=None, format=None):
    if track == "🎨 이미지 프롬프트" or track == "🖌️ 이미지 프롬프트 생성기":
        ...
        **한글 요약 설명:** [주제와 프롬프트의 의도, 전체 분위기, 핵심 요소를 2~3문장으로 요약]

        **프롬프트 요소:**
        - 주제(Topic): [주제]
        - 배경(Background): [배경/장소/환경]
        - 스타일(Style): [스타일]
        - 구도/앵글(Angle): [구도/앵글]
        - 색감(Colors): [색감/톤]
        - 조명(Lighting): [조명]
        - 분위기(Atmosphere/Mood): [분위기]
        - 추가 옵션(Additional Options): [고품질, 고해상도, 디테일 등]
        - 추가 지시사항: [특별한 요청사항]
        ...
        * [이미지 생성 시 도움이 될 만한 팁 4]
        ...
        ## 이미지 프롬프트 작성 시 고려할 요소:
        - 장면의 전체적인 분위기와 스토리텔링
        - 색감, 조명, 구도, 스타일의 조화
        - 등장인물, 사물, 배경의 디테일한 묘사
        - 감정과 분위기를 강화하는 시각적 요소(빛, 그림자, 질감 등)
        - 추가 옵션(고품질, 고해상도, 디테일 등) 활용
