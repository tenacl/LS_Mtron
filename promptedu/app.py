        ...
        for i, model_name in enumerate(GEMINI_MODELS):
            try:
                ...
                if i < len(GEMINI_MODELS) - 1 and is_quota_exceeded_error(error_msg):
                    ...
                    st.error("⚠️ 프롬프트 생성에 실패했습니다.")
                    if is_quota_exceeded_error(error_msg):
                        st.info(
                            "현재 AI 프롬프트 생성 요청이 많아 일시적으로 서비스가 제한되고 있습니다.\n\n"
                            "- Google Gemini API의 무료 사용량 한도를 초과했습니다.\n"
                            "- 1~2분 후 다시 시도해보시거나, 잠시 후 재접속 부탁드립니다.\n"
                            "- 계속 문제가 발생하면 관리자에게 문의해 주세요.\n\n"
                            "[자세히 보기](https://ai.google.dev/gemini-api/docs/rate-limits)"
                        )
                    else:
